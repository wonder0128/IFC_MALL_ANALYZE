...
var gl_arr_fnb_store_list = new Array();
...
function setInitSetting(p_result){
    ...
    $(".btn_left_type").bind("touchstart", function (e) {
        onClickSelectType(this);
    });

    $(".btn_right_type").bind("touchstart", function (e) {
        onClickSelectKeyword(this);
    });

    /*

    $("#id_lang_list_kor li").bind("touchstart", function (e) {
        onClickSearchKeyWord(this,"KOR");
    });

    $("#id_lang_list_eng li").bind("touchstart", function (e) {
        onClickSearchKeyWord(this,"ENG");
    });

    $("#id_lang_list_num li").bind("touchstart", function (e) {
        onClickSearchKeyWord(this,"NUM");
    });
    ...
    */
    ...
    if(parent.MAINPARENTCUSTOMCODE){
        ...
        for(i = 0; i < gl_xml_conf.xml_data.arr_fnb_store_list.length; i++){
            obj = gl_xml_conf.xml_data.arr_fnb_store_list[i];
...
function setInitConfig(p_load_data){
    ...
    var store_list = p_load_data.arr_store_list.filter((store) => store.CATE_CODE == 'S02');

    // F&B S09
    for(i = 0; i < store_list.length; i++){
        gl_arr_fnb_store_list.push(store_list[i]);
    }

    gl_arr_fnb_store_list.sort(function(a, b) { // 오름차순
        return a.STORE_NAME_KOR < b.STORE_NAME_KOR ? -1 : a.STORE_NAME_ENG > b.STORE_NAME_ENG ? 1 : 0;
    });
...
function setMainLang(p_type, p_lang){
    ...
    // 다국어 매장 이름 변환
    for(i = 0; i < gl_arr_fnb_store_list.length && i < 3000; i++){
        obj = gl_arr_fnb_store_list[i];
//         console.log('obj', obj);
        try{
...
function setInitMakeStoreList(){
    ...
    var i_total_page = Math.floor(gl_arr_fnb_store_list.length / gl_paging_conf.list_cnt) + 1;
    ...
    for(i = 0; i < gl_arr_fnb_store_list.length; i++){
        obj = gl_arr_fnb_store_list[i];
        str_img = "";
        ...
        str_html += "   <div class='logo_main' onClick='javascript:onClickStoreInfo(event,\"" + obj.id + "\",\"" + obj.STORE_FLOOR + "\");'>"+str_img;
        str_html += "   </div>";
        str_html += "   <div class='desc_main' onClick='javascript:onClickStoreInfo(event,\"" + obj.id + "\",\"" + obj.STORE_FLOOR + "\");'>";
        ...
        str_html += "                   <div class='floor_input' style='font-size: 12px;'>"+obj.STORE_FLOOR+"</div><br>";
        str_html += "               </div>";
    ...
    gl_paging_conf.total_cnt = gl_arr_fnb_store_list.length;
